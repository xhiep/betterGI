        ...
        get
        {
            if (_layers.Count == 0)
            {
                lock (_layersLock)
                {
                    if (_layers.Count == 0)
                    {
                        TaskControl.Logger.LogInformation("[TemplateMatch]Đang tải mẫu bản đồ Teyvat, có thể mất khá nhiều thời gian, vui lòng chờ...");
                        Layers = BaseMapLayerByTemplateMatch.LoadLayers(this);
                        TaskControl.Logger.LogInformation("Tải mẫu bản đồ hoàn tất!");
                    }
                }
            }
            return _layers;
